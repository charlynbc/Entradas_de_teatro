...
async function loadFuncionesHoy() {
    ...
    try {
        ...
        const funcionesHoy = shows.filter(show => {
            const showDate = new Date(show.fecha);
        ...
        grid.innerHTML = `
            <div class="no-funciones">
                ...
                <p>Error al cargar las funciones de hoy</p>
            </div>
        `;
    }
}
...
async function loadProximasFunciones() {
    ...
    try {
        ...
        const proximasFunciones = shows
            .filter(show => new Date(show.fecha) >= now)
        ...
        if (proximasFunciones.length === 0) {
            grid.innerHTML = `
                <div class="no-funciones">
                    ...
                    <p>No hay funciones próximas programadas</p>
                </div>
            `;
        } else {
            grid.innerHTML = '';
        ...
        grid.innerHTML = `
            <div class="no-funciones">
                <i class="fas fa-exclamation-triangle"></i>
                <p>Error al cargar las próximas funciones</p>
            </div>
        `;
    }
}
